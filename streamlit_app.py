...
def load_and_process_data(_processor, file_id):
	loader = DataLoader()
	raw_data = loader.from_gdrive(file_id=file_id)
...
raw_data['cluster_names'] = raw_data['clusters'].apply(lambda x: "cluster_" + str(x))
...
def draw_strip_range_by_clusters(df, column: str):
	...
	sales_dist_by_clusters['avg'] = df.groupby(by=['clusters'])[column].transform('mean')
...
with main_box:
	...
	"""This analysis from superstore selling data over 5 years. 
	...
	with distribution_box:
		...
		with col1:
			data_point_distribution_chart = px.scatter(
				...
				title='Data Point Distribution After Decomposition',
				subtitle='PCA Version of Data to be Prepared for Clustering',
	...
	with st.container(width='stretch'):
		cols = st.columns(2)
		with cols[0]:
			avg_discount_vs_price_chart = draw_avg_discount_vs_profit_by_clusters(clusters_summary)
